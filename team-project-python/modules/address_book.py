...
class AddressBook(UserDict):
    ...
    def show_birthdays(self, period):
        ...
        for name, contact in self.data.items():
            ...
            if isinstance(birthday, Birthday):
                ...
                birthday_this_year = datetime.strptime(str_time, "%d-%m-%Y").replace(
                    year=today.year
                )
        ...
        sorted_data = dict(
            sorted(grouped_birthdays.items(), key=lambda x: week_days.index(x[0]))
        )
        output = ""
    ...
    def search(self, input):
        matching_contacts = [
            contact for contact in self.data.values() if contact.contains_value(input)
        ]
